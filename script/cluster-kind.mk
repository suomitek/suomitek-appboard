...
${CLUSTER_CONFIG}:
	kind create cluster \
		...
		--name ${CLUSTER_NAME} \
		--config=./docs/user/manifests/suomitek-appboard-local-dev-apiserver-config.yaml \
		--retain \
		--wait 10s
	kubectl apply --kubeconfig=${CLUSTER_CONFIG} -f ./docs/user/manifests/suomitek-appboard-local-dev-users-rbac.yaml
...
devel/dex.crt:
	kubectl -n kube-system cp kube-apiserver-suomitek-appboard-control-plane:etc/kubernetes/pki/apiserver.crt ./devel/dex.crt

devel/dex.key:
	kubectl -n kube-system cp kube-apiserver-suomitek-appboard-control-plane:etc/kubernetes/pki/apiserver.key ./devel/dex.key

${ADDITIONAL_CLUSTER_CONFIG}: devel/dex.crt
	kind create cluster \
		--kubeconfig ${ADDITIONAL_CLUSTER_CONFIG} \
		--name ${ADDITIONAL_CLUSTER_NAME} \
		--config=./docs/user/manifests/suomitek-appboard-local-dev-additional-apiserver-config.yaml \
		--retain \
		--wait 10s
	kubectl apply --kubeconfig=${ADDITIONAL_CLUSTER_CONFIG} -f ./docs/user/manifests/suomitek-appboard-local-dev-users-rbac.yaml
	kubectl apply --kubeconfig=${ADDITIONAL_CLUSTER_CONFIG} -f ./docs/user/manifests/suomitek-appboard-local-dev-namespace-discovery-rbac.yaml
