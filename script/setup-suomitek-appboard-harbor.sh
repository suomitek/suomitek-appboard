#!/usr/bin/env bash

set -o errexit
set -o nounset
set -o pipefail

# Constants
ROOT_DIR="$(cd "$( dirname "${BASH_SOURCE[0]}" )/.." >/dev/null && pwd)"

# Load Libraries
# shellcheck disable=SC1090
. "${ROOT_DIR}/script/libtest.sh"
# shellcheck disable=SC1090
. "${ROOT_DIR}/script/liblog.sh"

info "Updating chart repositories..."
silence helm repo update
echo

# Install Harbor
info "-------------------------"
info "-- Harbor installation --"
info "-------------------------"
echo
"$ROOT_DIR"/script/setup-harbor.sh --namespace "harbor" --disable-clair --disable-notary
# Install Suomitek-appboard
info "---------------------------"
info "-- Suomitek-appboard installation --"
info "---------------------------"
echo
"$ROOT_DIR"/script/setup-suomitek-appboard.sh --namespace "suomitek-appboard" --initial-repos "harbor-library" "http://harbor.harbor.svc.cluster.local/chartrepo/library"
