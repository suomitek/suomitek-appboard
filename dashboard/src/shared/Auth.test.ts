  ...
  it("does not assume to be authenticated from the auth proxy if the message contains a service account", () => {
    expect(
      Auth.is403FromAuthProxy({
        status: 403,
        data:
          'namespaces is forbidden: User "system:serviceaccount:suomitek-appboard:suomitek-appboard-internal-kubeops" cannot list resource "namespaces" in API group "" at the cluster scope',
